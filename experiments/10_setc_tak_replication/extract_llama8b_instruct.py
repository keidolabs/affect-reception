"""
Experiment 10 — Set C Tak Replication: Activation Extraction (Llama-3-8B Instruct)
...
Output: outputs/activations/llama8b_instruct_set_c_residuals.npy — shape (80, 32, 4096)

Run: uv run python experiments/10_setc_tak_replication/extract_llama8b_instruct.py
"""
...
behavioral_lines = ["Set C Behavioral Check — Meta-Llama-3-8B-Instruct", "=" * 50, "",
...
(OUT_DIR / "behavioral_check_llama8b_instruct.txt").write_text("\n".join(behavioral_lines))
console.print("✓ behavioral_check_llama8b_instruct.txt")

# ============================================================================
# SAVE
# ============================================================================

np.save(ACT_DIR / "llama8b_instruct_set_c_residuals.npy", all_activations)
console.print(f"\n[bold green]✓ Saved: llama8b_instruct_set_c_residuals.npy ({all_activations.shape})[/bold green]")

with open(ACT_DIR / "llama8b_instruct_metadata.json", "w") as f:
    json.dump(metadata, f, indent=2)

import polars as pl
pl.DataFrame(metadata).write_csv(ACT_DIR / "llama8b_instruct_metadata.csv")
