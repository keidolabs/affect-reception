"""
Experiment 05 — Phase 1: Activation Extraction
Model: meta-llama/Llama-3.2-1B (16 layers, d_model=2048)
Date: 2026-02-24

Extracts final-token residual stream at all 16 layers for all 192 Set B stimuli
(96 clinical vignettes + 96 matched neutral controls).
Output shape: (192, 16, 2048)

Uses TransformerLens on CPU — same method as Phase 0 (00_phase0_replication)
for methodological consistency in the cross-phase comparison.

Estimated time: ~2-3 min on CPU (1B model, 192 stimuli)
Run: uv run python experiments/05_phase1_llama1b/extract.py
"""
...
DEVICE = "cpu"
DTYPE = torch.float32   # TL on CPU — float32 for correctness (MPS has TL issue #1178)
...
set_b_clinical = load_set_b_clinical()   # 96 stimuli, labeled by emotion
set_b_neutral  = load_set_b_neutral()    # 96 matched neutral controls
...
# ============================================================================
# LOAD MODEL (TransformerLens, CPU — same as Phase 0)
# ============================================================================
...
for stim_idx, stimulus in enumerate(track(all_stimuli, description="Extracting...")):
    ...
    for layer_idx in range(n_layers):
        act = cache["resid_post", layer_idx][:, -1, :].squeeze().cpu().to(torch.float32).numpy()
