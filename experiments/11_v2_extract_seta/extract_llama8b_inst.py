"""
Experiment 11 — v2 Set A Extraction: Meta-Llama-3-8B-Instruct
Question: How does RLHF affect emotion circuit structure in the 8B model?
Date: 2026-02-25

Framework: HF transformers + register_forward_hook on MPS
Run: uv run python experiments/11_v2_extract_seta/extract_llama8b_inst.py
"""
...
ACT_DIR = EXP_DIR / "outputs" / "activations" / "llama8b_inst"
...
assert len(set_a_emotional) == 80, f"Got {len(set_a_emotional)}"
...
console.print(f"Final token: {repr(final_tok)} — {'✓' if ':' in final_tok else '✗ WARNING'}")

# MPS validation
for prompt_text, acceptable in [
    ("The Eiffel Tower is located in the city of", ["Paris", " Paris"]),
    ("The boiling point of water is 100 degrees", ["Celsius", " Celsius", "C"]),
]:
