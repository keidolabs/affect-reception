"""
Experiment 11 — v2 Set A Extraction: Gemma-2-9B-IT (instruct)
Question: How does RLHF affect Gemma's emotion circuit structure?
Date: 2026-02-25

Framework: HF transformers + register_forward_hook on MPS
Run: uv run python experiments/11_v2_extract_seta/extract_gemma9b_inst.py
"""
...
for prompt_text, acceptable in [("The Eiffel Tower is located in", ["Paris", " Paris"]),
                                  ("Water boils at 100 degrees", ["Celsius", " Celsius"])]:
    ...
    console.print(f"  {'✓' if found else '✗'} device: {top5[:3]}")
...
for stimulus in track(set_a_emotional, description="Extracting..."):
    ...
    with torch.no_grad():
        try:
            ...
            console.print(f"[yellow]  OOM attn {stimulus.id} — zeros[/yellow]")
