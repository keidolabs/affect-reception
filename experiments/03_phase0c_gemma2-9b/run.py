...
ax.plot(layers, layer_aurocs, "o-", color="#9b59b6", linewidth=2, markersize=4,
        label="Gemma-2-9b")
...
# Mark Gemma-2 alternating attention layers (even=local, odd=global)
for li in range(0, n_layers, 2):  # even layers = local attention
    ax.axvspan(li - 0.4, li + 0.4, alpha=0.05, color="#9b59b6", zorder=0)

ax.set_xlabel("Layer (shaded = local attention layers)")
ax.set_ylabel("Macro AUROC")
ax.set_title(f"Phase 0c: Layer-wise Probe — {MODEL_ID}\nSet A (n=80 emotional), 8-class OvR, 5-fold CV")
ax.set_xlim(-0.5, n_layers - 0.5); ax.set_ylim(0, 1.05)
ax.set_xticks(range(0, n_layers, 4)); ax.legend(fontsize=8); ax.grid(alpha=0.3)
plt.tight_layout()
...
**Best layer: {best_layer}** ({"local" if best_layer % 2 == 0 else "global"} attention) — AUROC = **{best_auroc:.4f}** ± {layer_auroc_stds[best_layer]:.4f}
