"""
Experiment 12 — v2 Set B Extraction: Gemma-2-9B (base)
Date: 2026-02-25
Framework: HF transformers + register_forward_hook on MPS
Note: Gemma-2 alternating local/global attention; OOM fallback for attention patterns
Run: uv run python experiments/12_v2_extract_setb/extract_gemma9b_base.py
"""
...
console.print(f"[bold green]✓ {MODEL_ID} — {n_layers}L (alternating local/global)[/bold green]")

for p, acc in [("The capital of France is", ["Paris", " Paris", "paris"])]:
